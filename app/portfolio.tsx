...
const argCaseCatalog = [
  {code:"AGV-0528", title:"凌晨 3:17 的电梯", author:"灰塔记录员", type:"都市异闻 / 现场解谜", tag:"高危", heat:"2,418", description:"每晚 3:17，电梯会停在不存在的 13 层。有人从里面回来，却说自己没有上过楼。"},
  {code:"AGV-0831", title:"镜中人停在第十三秒", author:"白噪档案室", type:"网页互动解谜", tag:"新入档", heat:"1,036", description:"一段浴室镜面录像里，镜中人比现实晚了十三秒抬头。"},
  {code:"AGV-1013", title:"空号来电", author:"夜班接线员", type:"互动叙事 / 灵异事件", tag:"心理压力", heat:"4,812", description:"手机每天收到同一个空号的来电，接通后却是你未来三分钟后的声音。"},
  {code:"AGV-1124", title:"走廊尽头的第四扇门", author:"门牌观察组", type:"空间异常 / 网页解谜", tag:"高危", heat:"3,706", description:"老公寓的平面图只有三扇门，凌晨拍下的走廊却总会多出第四扇。"},
  {code:"AGV-0926", title:"太平间的值班灯", author:"白班交接处", type:"都市异闻 / 档案调查", tag:"限制级", heat:"2,109", description:"停用三年的太平间每天凌晨亮起一盏灯，值班表上却多出一个从未入职的人名。"},
  {code:"AGV-1207", title:"雨夜无脸乘客", author:"末班车记录员", type:"现实追踪 / 互动叙事", tag:"现实延伸", heat:"5,284", description:"末班公交每晚多载一位没有脸的乘客，车载录像里只有他的湿脚印。"},
  {code:"AGV-0414", title:"凌晨 4:44 的收音机", author:"静电监听组", type:"音频异常 / 网页调查", tag:"新入档", heat:"864", description:"一台断电十年的收音机每天 4:44 播放一段只对一个人说的天气预报。"},
  {code:"AGV-0618", title:"照片里多出的第九个人", author:"旧影像修复处", type:"影像取证 / 互动解谜", tag:"视觉异常", heat:"1,772", description:"一张毕业合照只有八名学生，放大后却在窗户反光里出现第九张脸。"},
  {code:"AGV-0730", title:"档案室的倒置页码", author:"灰纸档案室", type:"文档异常 / 逻辑调查", tag:"限制级", heat:"1,248", description:"封存卷宗的页码每天倒退一页，缺失的那一页写着调查员尚未做出的决定。"}
];
...
const projects: Project[] = [
  ...
  { id:"arg", name:"迷境 ARG 调查局", en:"MIJING ARG COMMUNITY", pitch:"玩家扮演调查员，从都市异闻、影像异常与文档缺页中整理可复核的证据链。", status:"在线整合", type:"ARG / 互动叙事", role:"叙事系统 / 调查流程 / 网页交互", tags:["ARG","互动叙事","证据链","社区协查"], image:"/media/projects/arg/01.webp", accent:"#c77b68", playable:true, playableHref:"https://mijing-arg-community.pages.dev/", playableLabel:"进入 ARG 调查局" },
...
const docsByProject: Record<string, DocSummary[]> = {
  ...
  arg: [
    { title:"ARG 灵异事件调查整合网站项目概览", type:"关卡与叙事", status:"在线整合", problem:"如何把多个灵异事件、玩家调查和社区协查组织成一个可以持续扩展的 ARG 入口？", conclusions:["用案件大厅承载不同类型的都市异闻与现实追踪","每个案件都保留状态、编号、简报和证据链入口","调查员协查频道把个人推理转为可回传、可复核的社区记录"], evidence:"G:/ARG 的 src/main.jsx 已包含调查终端、案件大厅、案件详情与协查结构；线上项目为 mijing-arg-community.pages.dev。", read:"6 min" },
    ...
    {selected&&<div className="modalBackdrop" role="presentation" onMouseDown={()=>setSelected(null)}>
      <article className="projectModal" role="dialog" aria-modal="true" aria-labelledby="modal-title" onMouseDown={e=>e.stopPropagation()}>
        ...
        <div className="modalBody">
          <aside><a href="#overview">概览</a>{selected.playableHref?<a className="asidePlay" href={selected.playableHref} target="_blank" rel="noreferrer">↗ {selected.id==="arg"?"进入 ARG 调查局":"立即在线试玩"}</a>:null}{projectDownloads.length?<a className="asideDownload" href="#downloads">↓ 下载试玩</a>:null}{selected.id==="arg"?<a className="asideArg" href="#arg-cases">▦ 案件内容目录</a>:null}<a href="#loop">核心玩法</a><a href="#media">美术资产</a><a href="#documents">项目文档</a><a href="#progress">当前进度</a><a href="#ai">AI 参与</a></aside>
          <div>
            ...
            {selected.id==="arg"&&<section id="arg-cases" className="argCaseArchive"><div className="argCaseHeading"><div><p className="kicker">CASE LIBRARY / FROM G:\ARG</p><h3>案件内容目录</h3><p>这不是普通项目介绍，而是一座可继续扩展的调查终端：玩家以调查员身份进入案件、保全证据、参与协查，再把事实与推测分开写入结案记录。</p></div><strong>{argCaseCatalog.length}<small> CASES</small></strong></div><div className="argCaseGrid">{argCaseCatalog.map((item,index)=><article key={item.code}><div className="argCaseMeta"><span>{String(index+1).padStart(2,"0")} / {item.code}</span><em>{item.tag}</em></div><h4>{item.title}</h4><p>{item.description}</p><div className="argCaseFoot"><span>{item.author}</span><span>{item.type}</span><b>热度 {item.heat}</b></div></article>)}</div><a className="argCaseLaunch" href="https://mijing-arg-community.pages.dev/" target="_blank" rel="noreferrer"><span>INVESTIGATOR ACCESS</span><strong>打开完整 ARG 调查员终端 ↗</strong><small>案件大厅、调查字段、证据链、协查记录与结案报告均在独立站点中运行。</small></a></section>}
